...
router.use(authenticate, requireFeatureEnabled('chat'));
router.get('/conversations', authenticate, async (req, res) => {
  try {
    const convs = await Conversation.find({
      participants: req.user._id
    }).populate('participants', 'name username role').populate({
      path: 'lastMessage',
      populate: {
        path: 'sender',
  ...
  try {
    ...
    if (!participantIds || !Array.isArray(participantIds) || participantIds.length === 0) {
      return res.status(400).json({
        message: 'participantIds required'
      });
    }
...
router.get('/conversations/:id/messages', authenticate, async (req, res) => {
  try {
    ...
    const messages = await Message.find({
      ...
      isDeleted: false
    }).populate('sender', 'name username role').populate('forwardedFrom', 'text').populate('deletedBy', 'name username').sort({
      createdAt: 1
...
router.get('/users', authenticate, async (req, res) => {
  try {
    ...
    const users = await User.find(query).select('name username role').sort({
      name: 1
    });
    res.json(users);
  } catch (err) {
...
router.post('/conversations/:convId/forward', authenticate, async (req, res) => {
  try {
    ...
    if (!messageId || !targetConvId) {
      return res.status(400).json({
        message: 'messageId and targetConvId required'
      });
    }
    const originalMsg = await Message.findById(messageId).populate('sender');
    if (!originalMsg) return res.status(404).json({
      message: 'Message not found'
    });
    ...
    if (!sourceConv.participants.includes(req.user._id) || !targetConv.participants.includes(req.user._id)) {
      return res.status(403).json({
        message: 'Not a participant in these conversations'
      });
    }
...
router.delete('/conversations/cleanup-orphaned', authenticate, async (req, res) => {
  try {
    if (!['admin', 'super_admin'].includes(req.user.role)) return res.status(403).json({
    ...
    const orphanedIds = allConvs.filter(c => !c.participants.some(p => allUserIds.has(p.toString()))).map(c => c._id);
    ...
    res.json({
      message: `Removed ${orphanedIds.length} orphaned conversation(s)`,
