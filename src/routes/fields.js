...
const EXTENDABLE_FIELDS = ['level', 'gsSubmission', 'olRequest', 'offerLetter', 'withdraw', 'payment', 'coeCas', 'savisFee', 'refund', 'visaOutcome', 'visaWithdraw'];
...
router.post('/options', requireAdmin, async (req, res) => {
  try {
    ...
    if (!fieldKey || !EXTENDABLE_FIELDS.includes(fieldKey)) return res.status(400).json({
    ...
    let config = await FieldConfig.findOne({
      kind: 'options',
      fieldKey
    });
    if (!config) {
      ...
      if (config.options.some(o => o.toLowerCase() === trimmed.toLowerCase())) return res.status(409).json({
...
router.delete('/options', requireAdmin, async (req, res) => {
  try {
    ...
    if (config.options.length === before) {
      ...
      if (!alreadyRemoved) {
        config.removedDefaults = [...(config.removedDefaults || []), option];
      }
    }
    await config.save();
    res.json({
...
router.post('/custom', requireAdmin, async (req, res) => {
  try {
    ...
    const last = await FieldConfig.findOne({
      kind: 'custom'
    }).sort({
      order: -1
    });
    ...
    let slotKey = 'last';
    if (position === 'first') {
      const firstInSection = await FieldConfig.findOne({
        kind: 'custom',
      ...
      const afterField = await FieldConfig.findOne({
        kind: 'custom',
        _id: afterFieldId
      }).catch(() => null);
      if (afterField) {
    ...
    const all = await FieldConfig.find({
      kind: 'custom'
    }).sort({
      order: 1,
      createdAt: 1
    });
    await Promise.all(all.map((f, idx) => f.order === idx ? null : FieldConfig.updateOne({
      _id: f._id
    }, {
      order: idx
    })));
